...
class GraphMaker:
    ...
    def build_graph(
        ...
        edge_list = []
        existed_node_num = self.graph.number_of_nodes()
        num_nodes = len(features)
        node_idx = existed_node_num
        cum_distance = 0
...
def merge_runs(data, start_idx=0):
    ...
    for run_idx in range(len(data)):
        if len(data[run_idx]) == 0: continue
...
def segmentation(
    ...
    for run_idx in idx_per_runs:
        if idx_per_runs[0] == idx_per_runs[1]: continue
        node_range = idx_per_runs[run_idx]
        sub_graph = [node_range[0]]
        distance_interval = []
        total_distance = 0
        for node in range(node_range[0] + 1, node_range[1]):
    ...
    for sub_graph in tqdm(subgraph_node):
...
def extend_length(dataset, length):
    features = dataset['features']
    dataset['features'] = torch.zeros(features.shape[0], length, features.shape[2])
    dataset['features'][:, :features.shape[1], :] = features
    poses = dataset['poses']
    dataset['poses'] = torch.zeros(poses.shape[0], length, poses.shape[2])
    dataset['poses'][:, :poses.shape[1], :] = poses
    masks = dataset['masks']
    dataset['masks'] = torch.ones(masks.shape[0], length) == 1
    dataset['masks'][:, :masks.shape[1]] = masks
    return dataset
